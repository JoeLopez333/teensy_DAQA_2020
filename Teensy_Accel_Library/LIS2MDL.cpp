...
// I2C read/write functions for the LIS2MDL
        ...
        //temp[0] = subAddress;
        //temp[1] = data;
        //Wire1.transfer(address, &temp[0], 2, NULL, 0); 
		...
		Wire1.requestFrom(address, (size_t) 1);  // Read one uint8_t from slave register address 
        ...
        //Wire1.transfer(address, &subAddress, 1, dest, count);
		Wire1.beginTransmission(address);   // Initialize the Tx buffer
		Wire1.write(subAddress);            // Put slave register address in Tx buffer
		Wire1.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
		uint8_t i = 0;
